# Credit to claudeHify from GitHub
# The base of the code is from, although some parts are altered:
# https://github.com/claudeHifly/BipedalWalker-v3/blob/master/DQN/dqn_agent.py#L3
...
class Agent():
    ...
    def act(self, state, eps):
        ...
        #### OUR CHANGES MADE HERE ####
        if random.random() > eps:
            action_idx = np.argmax(action_values.cpu().data.numpy())
        else:
            # Instead of taking random values  when p > eps, take a random action from the
            # list of actions the teacher will perform 
